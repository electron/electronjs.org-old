...
class Jexodus extends EventEmitter {
  ...
  buildRoutes () {
    ...
    Object.keys(flat(this.pages))
      ...
      .forEach(key => {
        ...
        let permalink = get(this.pages, `${key}.data.permalink`)
  ...
  render (href) {
    ...
    let page = get(this.pages, key)
    let output = page.content

    // Wrap page in layout
    const layout = this.layouts[page.data.layout || 'default']
    if (layout) {
      output = layout.content.replace(/{{ ?content ?}}/, output)
    }

    // Inject includes
    Object.keys(this.partials).forEach(includeName => {
      const _include = this.partials[includeName]
    ...
    const context = {
      site: Object.assign(this.config, {data: this.data})
    }
